...
type Suggestion = {
  ...
  title: string;
  message: string;
  metadata: { kind?: string; sourceUrl?: string | null } | null;
};
export function Suggestions() {
  ...
  useEffect(() => {
    ...
    fetch("/api/suggestions", { signal: controller.signal })
      ...
      .catch(() => {
        if (!controller.signal.aborted) setError("Suggestions unavailable.");
      });
  ...
  async function change(item: Suggestion, dismissed: boolean) {
    ...
    try {
      ...
      setRemoved(dismissed ? item : null);
    } catch {
  ...
  return (
    <section className="border-t border-line pt-4 space-y-3">
      <h3 className="font-semibold">Automation inbox ({items.length})</h3>
      {error && (
      ...
      {items.map((item) => (
        <article key={item.id} className="border-b border-line pb-3">
          <div className="flex justify-between gap-2">
            <h4 className="font-medium break-words min-w-0">{item.title}</h4>
            <button
              disabled={busy}
          ...
          {item.metadata?.sourceUrl?.startsWith("https://") && (
            <a
              href={item.metadata.sourceUrl}
              target="_blank"
              rel="noreferrer"
              className="inline-flex gap-1 items-center text-sm text-accent"
            >
              Source
