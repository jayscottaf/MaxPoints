...
type ReminderPerk = PerkTerms & { id: string; name: string; usage: UsageValue[] }
export function planReminders(cards: { name: string; perks: ReminderPerk[] }[], timezone: string, preferences: unknown, now = new Date()) {
  ...
  return cards.flatMap(card => card.perks.flatMap(perk => {
    ...
    const kind = prefs.expiring && daysRemaining !== null && cadence.includes(daysRemaining) ? 'expiring' : prefs.available && summary.periodStart.slice(0, 10) === today ? 'available' : null
    return kind ? [{ perkId: perk.id, cardName: card.name, perkName: perk.name, remainingValue: summary.availableValue, periodEnd: summary.periodEnd, daysRemaining, kind }] : []
  }))
}
